...
void luu_file(Book s[],int n){
	system("cls");
	ofstream ofs ("books.dat");
	if(!ofs){
		cout<<"khong the ghi tep ";
		exit(1);
	}
	ofs << "\t\t\t\t ---------- THONG TIN CUA CAC QUYEN SACH ----------------- \ n \ n " ;
	ofs << setw( 12 ) << "ma |" ; 
	ofs << setw ( 20 ) << "name |" ; 
	ofs << setw ( 20 ) << "tac gia |" ; 
	ofs << setw ( 17 ) << "nha XB |" ; 
 	ofs << setw ( 9 ) << "nam XB |" ; 
 	ofs << setw ( 12 ) << "don gia |" ;
 	ofs << setw ( 12 ) << "thue VAT |" ; 
 	ofs << setw ( 12 ) << "so luong |" ; 
 	ofs << setw ( 15 ) << "thanh tien |" ; 
 	ofs << setw ( 15 ) << "thuc tien |" << endl ; 
	 for(int i=0;i<n;i++){
	 	ofs <<  setw( 11 ) << s[i].ma_sach << "|" ; 
		ofs << setw ( 19 ) << s [ i ].ten_sach << "|" ; 
		ofs << setw ( 19 ) << s [ i ].tac_gia << "|" ; 
		ofs << setw ( 16 ) << s [ i ].nha_XB << "|" ; 
		ofs << setw (  8 ) << s [ i ].nam_XB << "|" ; 
		ofs << setw ( 11 ) << s [ i ].don_gia << "|" ; 
		ofs << setw ( 11 ) << s [ i ].VAT << "|" ; 
		ofs << setw ( 11 ) << s [ i ].so_luong<<"|" ; 
		ofs << setw ( 14 ) << s [ i ].thanh_tien << "|" ; 
		ofs << setw ( 14 ) << s [ i ].thuc_tien << "|" ; 
		ofs << " \ n " ; 
	 } 
	 ofs.close();
	 cout<<"\nDa luu tep\n";
...
void doc_file(Book b[],int n){
	system ( " cls " ); 
	string  x ;  
	ifstream  ifs (" books.dat ");
	while(! ifs . eof () ){
		getline ( ifs , x ); 
		cout<< x << endl ; 
	} 
